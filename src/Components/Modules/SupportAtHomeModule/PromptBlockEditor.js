...
/* ===========================
   ✅ FABULOUS SMART PARSER
   - Splits only by real headings
   - Keeps numbered/bulleted lists inside the same section
=========================== */
const splitPromptIntoBlocks = (text) => {
  ...
  // ✅ ALL CAPS heading like: PURPOSE, PROCESS, DATA TYPES, etc.
  ...
  // ✅ Title heading (first line in prompt usually)
  const isTitleLine = (t) =>
    t.length <= 90 &&
    ...
    <div className="pbe-root">
      ...
      {tab === "visual" && (
        <div className="pbe-visual">
          {blocks.map((b, idx) => (
            <Block key={idx} block={b} onSave={(val) => handleSaveBlock(idx, val)} />
          ))}
        </div>
      )}
